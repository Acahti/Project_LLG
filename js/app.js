...
function createFolderAction() { const n = document.getElementById('new-folder-name').value.trim(); if(!n) return showToast("이름 입력"); state.inventory.push({ id:'f'+Date.now(), type:'folder', category: currentCategory, name: n, parentId: null, timestamp: Date.now() }); DataManager.save(state); updateInvRender(); closeModal('modal-create-folder'); }
...
function saveFolderAction() { if(!currentFolderId) return; const n = document.getElementById('edit-folder-name').value.trim(); if(!n) return showToast("이름 입력"); const f = state.inventory.find(i=>i.id===currentFolderId); f.name = n; DataManager.save(state); updateInvRender(); closeModal('modal-folder-settings'); showToast("변경됨"); }
function deleteFolderAction() { if(!currentFolderId) return; if(state.inventory.some(i=>i.parentId===currentFolderId)) return showToast("폴더가 비어있지 않습니다."); state.inventory = state.inventory.filter(i=>i.id!==currentFolderId); currentFolderId = null; DataManager.save(state); updateInvRender(); closeModal('modal-folder-settings'); showToast("삭제됨"); }
...
function deleteItemEdit() { openConfirmModal("삭제", "정말 삭제?", ()=>{ state.inventory = state.inventory.filter(x=>x.id!==editingItemId); DataManager.save(state); updateInvRender(); closeModal('modal-edit-item'); showToast("삭제됨"); }); }
...
function renderCharacter() {
    ...
    ['STR','DEX','INT','WIS','VIT'].forEach(c => { const d = document.createElement('div'); d.className = 'stat-item'; d.innerHTML = `<div class="stat-header" onclick="toggleStat('${c}')"><span style="color:${state.cores[c].color}">● ${state.cores[c].name}</span><span>Lv.${state.cores[c].level} ▼</span></div><div id="detail-${c}" class="stat-detail" style="display:none;"></div>`; l.appendChild(d); const b = d.querySelector(`#detail-${c}`); for(let m in state.masteries) { if(state.masteries[m].core !== c) continue; let sh = ''; for(let s in state.skills) if(state.skills[s].mastery === m && !state.skills[s].hidden) sh += `<div class="skill-row"><div style="flex:1">- ${state.skills[s].name} (Lv.${state.skills[s].level})</div><button class="btn-edit" onclick="openEditSkillModal('${s}')">✎</button></div>`; b.innerHTML += `<div class="mastery-header"><span class="mastery-title">${state.masteries[m].name}</span><button class="btn-edit" onclick="openEditMasteryModal('${m}')">✎</button></div>${sh||'<div style="font-size:0.8em;color:#555;padding:5px;">없음</div>'}`; } });
}
